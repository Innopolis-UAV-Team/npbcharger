...
class TestChargeStatusParser(unittest.TestCase):
    ...
    def test_common_charging_scenarios(self):
        ...
        test_cases = [
            ...
            ("CC Charging", ChargeStatus.CCM.value, ["Constant Current Mode"], False, False),
            ("CV Charging", ChargeStatus.CVM.value, ["Constant Voltage Mode"], False, False),
            ("Float Charging", ChargeStatus.FVM.value, ["Float Mode"], False, False),
            ("Fully Charged", ChargeStatus.FULLM.value, ["Fully Charged"], False, False),
            ("CC Timeout", ChargeStatus.CCTOF.value, ["CC Mode Timeout"], False, True),
            ("Critical Fault", ChargeStatus.NTCER.value, ["NTC Short Circuit"], True, False),
    ...
    def test_all_charging_modes_simultaneous(self):
        ...
        active_names = [state["name"] for state in result["active_states"]]
        expected_names = ["Constant Current Mode", "Constant Voltage Mode", "Float Mode"]
        self.assertEqual(sorted(active_names), sorted(expected_names))
